    ...
    <div className="py-8">
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-slate-500 mb-6">
          ...
          <Link href="/packages" className="hover:text-emerald-600">Packages</Link>
          <ChevronRight className="w-4 h-4" />
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-6 text-slate-600">
                ...
                <span className="flex items-center gap-2"><Clock className="w-5 h-5 text-emerald-600" />{pkg.duration}</span>
              </div>
            </div>
              ...
              <div className="bg-white rounded-2xl p-6">
                ...
                <div className="space-y-6">
                  {pkg.itineraries.map((day) => (
                    <div key={day.id} className="border-l-2 border-emerald-200 pl-4">
                      <h3 className="font-semibold text-emerald-600 mb-2">Day {day.day}{day.title && `: ${day.title}`}</h3>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {pkg.includedItems.length > 0 && (
                <div className="bg-white rounded-2xl p-6">
                  <h2 className="text-xl font-semibold mb-4 text-emerald-600">Included</h2>
                  ...
                  {pkg.meetingPoints.map((mp) => (
                    <div key={mp.id} className="flex items-start gap-3 p-3 bg-slate-50 rounded-lg">
                      ...
                      <div>
                        ...
                        {mp.time && <p className="text-sm text-emerald-600">Pickup time: {mp.time}</p>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="sticky top-24 bg-white rounded-2xl p-6 shadow-lg">
              ...
              {pkg.departures.length > 0 ? (
                <div className="space-y-4">
                  ...
                  <div className="space-y-3 max-h-75 overflow-y-auto">
                    {pkg.departures.map((dep) => {
                      ...
                      return (
                        <div key={dep.id} className={`p-4 border rounded-xl ${isFull ? "bg-slate-50 opacity-60" : "border-emerald-200 hover:border-emerald-400"} transition-colors`}>
                          ...
                          {pkg.tripType === "OPEN_TRIP" ? (
                            <>
                              <p className="text-lg font-bold text-emerald-600">{dep.pricePerPerson ? formatPrice(Number(dep.pricePerPerson)) : "Contact"}<span className="text-sm font-normal text-slate-500">/person</span></p>
                            ...
                            <div className="space-y-2">
                              {dep.groups.map((g) => (
                                <div key={g.id} className={`p-2 rounded-lg ${g.isBooked ? "bg-slate-100 opacity-60" : "bg-emerald-50"}`}>
                                  <div className="flex justify-between">
                                    <span className="text-sm">Group {g.groupNumber}</span>
                                    <span className="font-semibold text-emerald-600">{formatPrice(Number(g.price))}</span>
                                  </div>
                  ...
                  <Link
                    ...
                    className="block w-full py-3 px-4 bg-emerald-600 text-white text-center rounded-xl font-semibold hover:bg-emerald-700 transition-colors"
                  >
                    Book Now
                  </Link>
                </div>
                ...
                <div className="text-center py-8">
                  ...
                  <Link href="/custom-request" className="text-emerald-600 font-medium mt-2 inline-block">Request Custom Date</Link>
                </div>
              )}
            </div>
          </div>
