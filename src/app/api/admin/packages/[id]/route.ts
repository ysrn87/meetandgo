...
export async function PUT(
  ...
  try {
    ...
    const updatedPackage = await prisma.$transaction(async (tx) => {
      // Update basic info (without includedItems and excludedItems - they are relations)
      ...
      // Update included items - delete and recreate
      await tx.includedItem.deleteMany({ where: { tourPackageId: id } });
      if (includedItems && includedItems.length > 0) {
        await tx.includedItem.createMany({
          data: includedItems.map((item: string, index: number) => ({
            tourPackageId: id,
            item: item,
            order: index,
          })),
      ...
      // Update excluded items - delete and recreate
      await tx.excludedItem.deleteMany({ where: { tourPackageId: id } });
      if (excludedItems && excludedItems.length > 0) {
        await tx.excludedItem.createMany({
          data: excludedItems.map((item: string, index: number) => ({
            tourPackageId: id,
            item: item,
            order: index,
          })),
...
export async function DELETE(
  ...
  try {
    ...
    // Delete package and all related data (cascade should handle most)
