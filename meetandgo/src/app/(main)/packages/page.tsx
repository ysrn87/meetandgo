      ...
      <div className="container mx-auto px-4 lg:px-8">
        ...
        {packages.length > 0 ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {packages.map((pkg) => (
              <Link key={pkg.id} href={`/packages/${pkg.slug}`} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300">
                ...
                <div className="p-6">
                  <h3 className="text-xl font-semibold mb-2 group-hover:text-primary-600 transition-colors">{pkg.title}</h3>
          ...
          <div className="text-center py-20 bg-white rounded-2xl">
            ...
            <Link href="/packages" className="text-primary-600 font-medium hover:underline">Clear all filters</Link>
          </div>
        )}
      </div>
    </div>
