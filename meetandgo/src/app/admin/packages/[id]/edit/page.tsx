...
interface PackageData {
  ...
  isActive: boolean;
  highlights: { id: string; title: string; description: string | null; image: string | null; order: number }[];
  itineraries: { id: string; day: number; title: string | null; activities: { id: string; startTime: string; endTime: string; activity: string; description: string | null; order: number }[] }[];
  includedItems: string[];
  excludedItems: string[];
  meetingPoints: { id: string; name: string; address: string | null; time: string | null; order: number }[];
  departures: { id: string; departureDate: string; pricePerPerson: any; maxParticipants: number | null; _count: { bookings: number }; groups: { id: string; groupNumber: number; price: any; maxParticipants: number; isBooked: boolean }[] }[];
}
...
export default function EditPackagePage() {
  ...
  const [meetingPoints, setMeetingPoints] = useState<MeetingPointForm[]>([]);
  const [departures, setDepartures] = useState<DepartureForm[]>([]);
  ...
  useEffect(() => {
    async function fetchPackage() {
      try {
        ...
        setItineraries(data.itineraries.map(it => ({
          id: it.id,
          day: it.day,
        ...
        setIncludedItems(data.includedItems.length > 0 ? data.includedItems : [""]);
        setExcludedItems(data.excludedItems.length > 0 ? data.excludedItems : [""]);

        setMeetingPoints(data.meetingPoints.map(m => ({
          id: m.id,
          name: m.name,
          address: m.address || "",
          time: m.time || "",
        ...
        const deps = data.departures.map(d => ({
          id: d.id,
          departureDate: new Date(d.departureDate).toISOString().split("T")[0],
        ...
        // Check if any departure has bookings
        setHasAnyBookings(deps.some(d => d.hasBookings || d.groups.some(g => g.isBooked)));
